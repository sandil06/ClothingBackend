...
const MenJeans = () => {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setLoading(true);
        const res = await fetch(`${API_BASE}/api/menswear?type=jeans`);
        const data = await res.json();
        const list = data?.data || [];
        setJeans(list);
      } catch (e) {
        console.error('Failed to load mens jeans', e);
      ...
      <main className="flex flex-col items-center px-0 py-6">
        <div className="w-full max-w-7xl mx-auto px-6 md:px-10 border-x border-gray-800">
          <div className="text-sm text-gray-400 mb-2">
            <a href="/mens" className="hover:underline">Men</a> <span className="mx-1">/</span> <span className="text-white">Jeans</span>
          </div>
          <h1 className="text-3xl font-bold mb-4 text-white">Jeans</h1>
          ...
          >
            {jeans.map((item, i) => (
              <div key={i} className="flex flex-col gap-2" style={{ width: '200px' }}>
                <img
                  src={(item.images && item.images[0]) || 'https://via.placeholder.com/400x512?text=Jeans'}
                  alt={item.name}
                  style={{ width: '200px', height: '256px', objectFit: 'cover', borderRadius: '8px', backgroundColor: '#111' }}
                  onError={(e) => { e.currentTarget.src = 'https://via.placeholder.com/400x512?text=Jeans'; }}
                />
